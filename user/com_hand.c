...
*/
    ...
    // char help20[57] = {"Load R3 - Loads the R3 test processes from <processes.h>\n"};
    // char help21[53] = {"Yield - Causess command handler to yield to the CPU\n"};   
    // char help22[43] = {"Alarm - creates an alarm at a desired time\n"}; 
 ...
 //write startup page
    ...
    while (1){
        ...
        if(!strcmp(buff, "help")|| !strcmp(buff, "Help"))
        {
           ...
           sys_req(WRITE, COM1, "Please remember to release the shift key when pressing the enter key\n", 70);
        ...
        if (pcb1 && pcb2 && pcb3 && pcb4 && pcb5) {
           ...
           //Start R5 User Commands
            ...
            // { 
                ...
                // ra = atoi(readAddress);
                // vaddress = check_valid(ra);

                // if(ra == 0)
                // {
                //      sys_req(WRITE, COM1, "Invalid Address: Please enter a valid address.\n", 48);
                // }

                integerInput = reverseHex(hexToIntStr);
   
                // char* 
                // sys_req(WRITE,COM1, , 10);


            // }
            char str[11] = {0};
            itoa(integerInput, str);
            sys_req(WRITE,COM1,"Attempting to Memory address: ", 31);
            sys_req(WRITE,COM1, newline, 2);
            ...
            if(valid == 0)
            {
                sys_req(WRITE,COM1,"Memory freed\n", 14);
                // combine_free_blocks();
        ...
        else if(!strcmp(buff,"Show All Memory") ||!strcmp(buff,"SA") || !strcmp(buff,"show all") || !strcmp(buff,"Show All") || !strcmp(buff,"Show all") || !strcmp(buff,"showall") || !strcmp(buff,"ShowAll")){
            //show all processes
            show_allocated_memory();
            sys_req(WRITE,COM1, newline,2);
            sys_req(WRITE,COM1, newline,2);
